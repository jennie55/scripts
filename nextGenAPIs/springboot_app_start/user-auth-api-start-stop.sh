...
#The manage options are start, stop, status
...
#Script variables
JARFile="user-auth-api.jar"
PIDFile="user-auth-api-app.pid"
HOST_NAME=$(hostname)
JMX_OPTIONS="-Dcom.sun.management.jmxremote -Dcom.sun.management.jmxremote.port=1089 \
             -Dcom.sun.management.jmxremote.authenticate=false -Dcom.sun.management.jmxremote.ssl=false \
             -Djava.rmi.server.hostname=$HOST_NAME"
JVM_OPTIONS="-Xmx2g -Djava.security.egd=file:/dev/./urandom $JMX_OPTIONS"
SPRING_OPTIONS="--spring.config.location=config/app.properties,config/env.properties"
PORT_NUMBER=8080
...
case "$ACTION" in
  ...
  start)
    ...
    nohup java $JVM_OPTIONS -jar $JARFile $SPRING_OPTIONS > user-auth-api.out 2>&1 &
